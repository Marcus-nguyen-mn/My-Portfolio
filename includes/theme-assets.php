   ...
   $js_files = glob(THEME_DIR.'/assets/js/*.js');
    foreach ( $js_files as $file ) {
      $handle = $script_handle. '-' .basename($file, '.js');
        wp_register_script($handle, THEME_URL.'/assets/js/'.basename($file), array('jquery'), false, false);
        wp_enqueue_script($handle,null,microtime(),true);
    }
}
...
function theme_enqueue_styles()
{
    ...
    foreach ($css_files as $file) {
        ...
        wp_enqueue_style($handle,null,microtime());
    }
}
